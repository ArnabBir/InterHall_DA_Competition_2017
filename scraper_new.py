...
req = urllib2.Request('http://www.unitedstateszipcodes.org/92335/', headers={ 'User-Agent': 'Mozilla/5.0' })
...
for line in f1:
    ...
    if k>350:
    	if k<450:
    		if line.find("<th>")>-1:
    			if line.find("</th>")>-1:
    				print re.sub('[^A-Za-z0-9]+', '', line.strip()[4:-5]),
    				print ' ',
    		if line.find("<td class=")>-1:
    			if line.find("</td>")>-1:
    				print re.sub('[^A-Za-z0-9]+', '', line.strip()[23:-5]);
